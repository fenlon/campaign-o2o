...
public class QRCodeUtil {
	...
	private static BufferedImage getQR_CODEBufferedImage(
		...
		try {
			...
			Map<EncodeHintType, Object> hints = getDecodeHintType(code
					.getErrorLevel());
			// 参数分别为：编码內容，编码类型，生成图片宽度，生成图片高度，设置参数
			bm = multiFormatWriter.encode(code.getContent(), barcodeFormat,
			...
			for (int x = 0; x < code.getSize(); x++) {
				for (int y = 0; y < code.getSize(); y++) {
					image.setRGB(x, y, bm.get(x, y) ? code.getBackGroundColor()
							: code.getForegroundColor());
				}
			}
