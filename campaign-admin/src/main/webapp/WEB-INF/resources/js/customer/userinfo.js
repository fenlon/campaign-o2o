var userModule = angular
		...
		.factory('Store', [ '$resource', function($resource) {
			...
			return $resource(url, {
				...
				'query' : {
					url : storeUrl + "queryByUIdByPage.json",
					method : 'GET'
				},
				// 'query' : {
		...
		.controller(
				...
				function($scope, $routeParams, $location, Store) {
					...
					/*
					 * $scope.baseUrl =
					 * 'http://120.25.230.36:8080/campaign-qrcode/' +
					 * "/qrcode/get.html?content="; $scope.storeUrl =
					 * "http://192.168.1.207:8080" + ctx +
					 * "/store/mobile/storeRedWallUI.htm?id="; $scope.qrcodeUrl =
					 * 'http://120.25.230.36:8080/campaign-qrcode/' +
					 * "/qrcode/get.html?content=''";
					 */

					$scope.baseUrl = qrcodeUrl + "/qrcode/get.html?content=";
					$scope.storeUrl = webUrl
					...
					$scope.showQRCode = function(index) {
						...
						$scope.qrcodeUrl = $scope.baseUrl
								...
								+ "&size=450"
								+ "&logoUrl=http://120.25.230.36:8080/campaign-qrcode/resources/images/qr_logo/xmn.png";
		...
		.config(function($routeProvider, $locationProvider) {
			$routeProvider.when('/', {
				...
				templateUrl : '../store_campaign/detail.htm'
			}).when('/store', {
